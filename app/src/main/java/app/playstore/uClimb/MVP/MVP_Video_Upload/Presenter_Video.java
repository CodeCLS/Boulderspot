...
public class Presenter_Video {
    ...
    public Presenter_Video(String place_id, String grade, String impression, String type, String tries, String place_name, String info, Uri uri) {
    ...
    public void addSuccess(Context mContext, String random_hash, Boolean source_status, Boolean statistics_status, Uri Source) {
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.child("User").child(uid).child("Points").exists()) {
                    Double points = Double.parseDouble(dataSnapshot.child("User").child(uid).child("Points").getValue().toString());
                    Log.d(TAG, "points23: " + points);
                    if (source_status) {
                        Log.d(TAG, "Presenter_Video23");

                        writeData(databaseReference, uid, random_hash, currentDateandTime, statistics_id, statistics_status, Source, points, mContext);
    ...
    private void writeData(DatabaseReference databaseReference, String uid, String random_hash, String currentDateandTime, String statistics_id, Boolean statistics_status, Uri Source, Double points, Context mContext) {
        ...
        Toast.makeText(mContext, "Geuploaded", Toast.LENGTH_SHORT).show();


    }

    private void checkStatistics(DatabaseReference databaseReference, String statistics_id, Boolean statistics_status, String random_hash, String currentDateandTime, Uri Source, Double points, String uid) {
        if (statistics_status) {
            Log.d(TAG,"points23:4" + points + " system: " + points_system(grade));
